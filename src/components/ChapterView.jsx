...
export default function ChapterView({ subject, chapter }) {
  ...
  useEffect(() => { setSubtab('study'); }, [chapter.id]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await Promise.all(chapter.blocks.map(async (b) => {
        r[b.id] = await storage.get(`studyguide:${sid}:ch${chapter.id}:block${b.id}:reviewed`);
        c[b.id] = await storage.get(`studyguide:${sid}:ch${chapter.id}:block${b.id}:confidence`);
      }));
      await Promise.all(chapter.questions.map(async (_, i) => {
        q[i] = await storage.get(`studyguide:${sid}:ch${chapter.id}:q${i}`);
      }));
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <header className="mb-6">
        ...
        {chapter.subtitle && <p className="text-slate-600 mt-1 text-sm">{chapter.subtitle}</p>}
      </header>
        ...
        {SUBTABS.map((t) => (
          <button key={t.id} onClick={() => setSubtab(t.id)}
            className={`px-4 py-2 text-sm font-semibold border-b-2 -mb-px transition ${
              subtab === t.id ? 'border-sky-600 text-sky-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
            {t.label}
          </button>
        ))}
      </div>
                ...
                return (
                  <li key={b.id}>
                    <a href={`#block-${b.id}`} className="flex items-center gap-3 px-2 py-1.5 rounded hover:bg-slate-50">
                      <span className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${isReviewed ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-500'}`}>{isReviewed ? '✓' : '○'}</span>
                      <span className={`inline-block w-3 h-3 rounded-full ${conf ? confidenceColors[conf].split(' ')[0] : 'bg-slate-200'}`} />
                      <span className="text-sm text-slate-700 flex-1">{b.num && <span className="text-slate-400 mr-1">{b.num}</span>}{b.title}</span>
      ...
      {subtab === 'review' && <KeyReview keyReview={chapter.keyReview} chapterTitle={chapter.shortTitle} />}
        ...
        <div>
          <div className="bg-white border border-slate-200 rounded-xl p-5 mb-6 flex flex-wrap gap-4 items-center justify-between shadow-sm">
            <div>
              ...
              <p className="text-sm text-slate-600">{chapter.questions.length} discrete questions from Kaplan</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-center"><div className="text-2xl font-bold text-emerald-600">{correctCount}</div><div className="text-xs text-slate-500">Correct</div></div>
              <div className="text-center"><div className="text-2xl font-bold text-slate-900">{answeredCount}/{chapter.questions.length}</div><div className="text-xs text-slate-500">Answered</div></div>
              <div className="text-center"><div className="text-2xl font-bold text-sky-600">{answeredCount ? Math.round((correctCount / answeredCount) * 100) : 0}%</div><div className="text-xs text-slate-500">Score</div></div>
            </div>
          </div>
